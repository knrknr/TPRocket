...
import com.mrhi2022.tpquickplaceforkakaoknr.databinding.ActivitySignUpBinding
...
class SignUpActivity : AppCompatActivity() {
    ...
    //업 버튼 클릭시에 발동하는 콜백메소드
    override fun onSupportNavigateUp(): Boolean {
        onBackPressed()//백버튼 누르는거와 같음
    ...
    private fun clickSignup(){
        ...
        //유효성 검사 - 패스워드와 패스워드 화면이 맞는지 검사
        //코틀린에서는 문자열 비교시에 equals()대신에 ==[자동으로 내부적으로 equals()로 계산됨]을 사용하는 것을 권장
        if (password != passwordConfirm){
            AlertDialog.Builder(this).setMessage("password 확인에 문제가 있습니다. 다시 확인하여 입력해주시기 바랍니다.").create().show()
            binding.etPasswordConfirm.selectAll()
            return
        }

        //Firebase Firestore DB 관리 객체 얻어오기
            ...
            .get().addOnSuccessListener {
                ...
                if (it.documents.size>0){ //같은 이메일이 이미 있다는 것임
                    ...
                    db.collection("emailUsers").add(user).addOnSuccessListener {
                        ...
                        Toast.makeText(this, "회원 가입에 오류가 발생했습니다.\n다시 시도해주세요요", Toast.LENGTH_SHORT).show()
                    }
                }
            }.addOnFailureListener {
